...
//     빈 목록 40.9% → **21.4%** · 영문 입력 16.1% → **5.1%**
//     우리 사전만으로 열 줄이 차는 비율 **74.7%**
//
// ── 남은 21.4%는 무엇인가 (더 파기 전에 읽을 것) ─────────────────────────────
// 갈라 보니 **대부분 비는 게 맞는 것**이었다. 무리해서 더 채우려 들지 말 것.
//   2.5%  이미 정확히 있는 말을 다 친 경우 — 더 보여 줄 게 없다(정상)
//   4.1%  **카드 번호를 치는 중**("피카츄 0", "리자몽 02") — 우리 사전에 번호는 없다.
//         넣으려면 카드 40,489장을 다 담아야 하는데 그만한 값어치가 없다.
//   2.1%  **스니커덩크 문법을 치는 중**(":1", ":1E") — 이건 뜻이 완성되기 전이라
//         아무것도 안 보여 주는 게 맞다.
//  나머지 대부분도 "이미 정확히 있는 말"이거나 한두 가지 구멍이다:
//   · "캡틴피카츄 AR" — 캡틴피카츄는 저쪽 덤프의 레어도 줄에 없어 짝이 안 생긴다
//   · "맥도날드"(30번) — 영문 세트 McDonald's Collection의 **한글 이름이 없다**.
//     자동 번역기(koreanizeEnglishSetName)는 세트 **코드**만 알아서 못 바꾼다.
//     한글 이름을 지어내면 안 되니 사장님 확인이 필요하다.
//   · "플러시 C" — 플러시 카드가 우리 자료에 세 장뿐이다
//
...
//
// ⚠️ 도감 화면(PokedexView)의 거르기는 **다른 글자를 지운다**(가운뎃점 `·`, 대소문자).
//    같아 보이라고 맞추고 싶어지는데 **맞출 필요 없다** — 하는 일이 다르다. 저쪽은
//    "쳐 놓은 말로 목록을 좁히는" 것이고, 여기는 "같은 이름을 두 번 보여 주지 않는" 것이다.
