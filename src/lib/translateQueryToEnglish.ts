...
// 한글(일본판) 팩 이름 → 이베이가 쓰는 영문 세트명. 일본판과 영문판은 발매 단위가
// 1:1로 안 맞아서(일본판 두 팩이 영문판 한 세트로 묶이는 식) 가장 가까운 세트로 잇는다.
// 팩 이름을 친 사람은 "그 시기 그 카드들"을 찾는 것이므로 이 정도면 목적에 닿는다.
...
export function translateSearchQueryToEnglish(query: string): string {
  ...
  // 조각나 안 걸린다.
  for (const { re, en } of packEnPatterns) {
    re.lastIndex = 0;
    if (re.test(result)) {
      re.lastIndex = 0;
