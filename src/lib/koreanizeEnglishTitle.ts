...
export const STRUCTURAL_EN_TO_KO: [string, string][] = [
  ...
  //    덤프에는 "Captain Pikachu"로 들어 있는데 이 낱말이 안 바뀌어 "Captain 피카츄"로
  //    나갔다. 붙여 쓰는 표기라 뒤 공백을 두지 않는다(위 'Mega '와 같은 방식).
  //    덤프 전체를 훑어도 "Captain"이 든 이름은 이 카드 하나뿐이라 부딪힐 데가 없다.
  ['Captain ', '캡틴'],
  ...
  ['Koga’s ', '독수의 '],
  ['Giovanni’s ', '비주기의 '],
  // ⚠️ **성도(2세대) 인물이 통째로 빠져 있었다.** 관동 인물(웅·이슬·민화…)만 있어서
  //    저쪽에서 오는 이름이 "Morty’s 나인테일"처럼 **반만 옮겨져** 화면에 나갔다
  //    (2026-08-08, 사장님이 나인테일 화면에서 잡아 주셨다).
  //    ⚠️ 한글 이름은 **지어내지 않았다.** 우리 일본판 표가 이미 아는 값을 그대로 옮겼다
  //       (マツバ→유빈 · シジマ→사도 · ミカン→규리 …). 두 길이 같은 값을 내야
  //       같은 카드가 일본판 탭과 북미판 탭에서 같은 이름으로 보인다.
  ['Falkner’s ', '비상의 '],
  ['Bugsy’s ', '호일의 '],
  ['Whitney’s ', '꼭두의 '],
  ['Morty’s ', '유빈의 '],
  ...
  [' δ', ' (델타종)'],
  // ⚠️ **저쪽(PPT)은 δ 대신 낱말로 적는다** — "Gardevoir (Delta Species)".
  //    이 줄이 없으면 그대로 "가디안 (Delta Species)"가 나간다. 우리 세트 자료는 δ를
  //    쓰므로 위 줄만으로 됐던 것이고, 저쪽에서 온 이름은 여기서 걸린다.
  [' (Delta Species)', ' (델타종)'],
...
export function koreanizeEnglishCardName(rawName: string): string {
  ...
  let name = rawName.replace(/-(GX|EX|V|VMAX|VSTAR)(?=$|\s)/g, ' $1');
  // ⚠️ **저쪽은 ☆를 "Star"라고 적는다** — "Gyarados Star (Delta Species)".
  //    우리 세트 자료는 ☆를 쓴다("Umbreon ☆").
  //    ⚠️ **델타종 앞에서만** 바꾼다. 두 가지를 안 건드리려는 것이다:
  //       ① "Team Star Grunt"(스타단 조무래기)·"Star Piece"(별의 조각) — 딴 낱말이다.
  //       ② 골드스타 다섯 장('Groudon Star' → '그란돈 ★')은 **아래 표에 일부러
  //          ★로 적어 둔 것**이다. 여기서 ☆로 바꾸면 그 표를 못 찾고 덮어 버린다.
  //          (우리 자료는 ☆ 24장·★ 0장이라 표기가 갈려 있다 — 어느 쪽이 맞는지는
  //           사람이 정할 일이라 건드리지 않는다.)
  name = name.replace(/ Star(?= \(Delta Species\))/, ' ☆');
  // ⚠️ **괄호 꼬리는 들머리에서 바꿔야 한다.** 아래 "이름 (괄호)" 갈래는 괄호 앞부분만
  //    사전에서 찾고 **괄호는 그대로 도로 붙인다**. 그래서 뒤쪽 낱말 규칙까지 못 가고
  //    "가디안 (Delta Species)"가 나갔다 — 사전에 "Gardevoir"가 있어서 걸린 것이다.
  //    사전에 없는 이름("Sceptile ex")은 규칙까지 내려가 멀쩡했으니, 있는 이름만
  //    깨지는 셈이라 더 눈에 안 띄었다.
  name = name.replace(/ \(Delta Species\)/g, ' (델타종)');
