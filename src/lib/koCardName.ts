...
// ⚠️ **괄호가 뒤따르는 꼴도 뗀다** — `Mew ex - 205/165 (151 Metal Card)`.
//    맨 뒤만 보던 탓에 이런 이름 2,745개는 번호가 그대로 남아 화면에 두 번 나왔다
//    (2026-08-29). 이름 끝에 붙은 번호는 원래도 떼고 있었으니, 예외를 없애는 것이다.
const 번호꼬리떼기 = (s: string) =>
  s
    .replace(/\s*-\s*\d+\/\d+\s*(?=\(|$)/, ' ')
    .replace(/\s+/g, ' ')
    .trim();
