...
// ⚠️ 이름은 **여기서** 다듬는다. 부르는 쪽(스니커덩크·이베이·TCGplayer 상세) 셋이
//    제각각 넘기던 것을 한 곳으로 모았다 — 스니커덩크는 원본 제목을 통째로 넘겨
//    공유 시트에 "…[M6 113/076](확장팩「…」) 시세"가 떴다(2026-08-07 발견).
//    링크를 받은 사람이 보는 탭 제목과 같은 규칙이어야 한다.
  ...
  return (
    <button
      ...
      onClick={async () => {
        trackEvent('share');
        if (navigator.share) {
          navigator.share({ title: `${이름} 시세 | pokegre`, url }).catch(() => undefined);
          return;
        }
        try {
