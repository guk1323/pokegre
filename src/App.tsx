    ...
    const timer = setTimeout(() => {
      ...
      searchEbayCards(trimmed, edition, 0, market, 도감 ? pptSetName(도감) : undefined)
        .then(({ cards, hasMore, translated, asOf }) => {
          ...
          // 번호는 맞는데 이름이 다르면 그 번호의 **다른 인쇄**만 값이 있는 것이다
          //  (블랙볼트 56번은 "마스터볼 패턴"만 값이 있다). 말없이 열면 눌렀던 그림과
          //  다른 카드가 나온 것처럼 보인다(점검 중 발견 2026-08-06).
          const 다른인쇄 = 그카드 && !이름까지맞음 ? 그카드.name : '';
          ...
          if (도감) {
            ...
            set도감안내(
              ...
              cards.length === 0
                ...
                : 다른인쇄
                  ? `${짧은세트(도감.setNameKo)} ${도감.num}번은 “${다른인쇄}”만 값이 있어 그걸 보여 드립니다.`
                : !고를것
