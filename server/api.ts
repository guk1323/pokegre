...
읽는 순서:
1) 카드가 등급 케이스에 들어 있고 위쪽에 영어 라벨이 보이면, 세트 코드와 카드 번호를 그 라벨에서 먼저 읽어라(작고 반짝이는 카드 글씨보다 라벨이 또렷하다).
2) 이름은 카드에 인쇄된 일본어를 그대로 읽어라.
3) 홀로그램 반사·번들거림이 있으면, 반사에 가려지지 않은 또렷한 글자만 읽어라.

{"found": true, "pokemonNameJa": "카드에 인쇄된 이름을 일본어 그대로. ex·V·VMAX·VSTAR·GX·SAR 같은 표기가 붙어 있으면 반드시 포함(예: ゲッコウガex, ピカチュウV). 큰 글씨라 반사에도 잘 보인다", "cardNumber": "카드 번호(예: 090/066, 209/XY-P, 025/165). 라벨이나 카드에서 읽되, 반사로 흐릿해 확실치 않으면 절대 지어내지 말고 null. 틀린 번호보다 null이 낫다", "setCode": "세트 코드(예: SV5a, XY-P, M4). 라벨이나 카드에서. 안 보이면 null"}
