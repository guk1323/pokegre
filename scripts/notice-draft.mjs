...
- "카드 이름/제목 한글화·일본어·한자 번역 표기 다듬기"처럼 상시로 조금씩 계속하는 유지보수성 작업은 제외하세요. 매일 반복돼 공지로서 의미가 없습니다. (한글화 사전 보정, 번역 표기 수정 등은 넣지 마세요.)
- 새 기능이거나, 특정 문제를 확실히 고친 눈에 띄는 변화만 넣으세요.
- 과장하거나 미래를 약속하지 마세요. "무료 / 영구 / 광고 없음 / 비영리" 같은 미래를 못 박는 표현은 쓰지 마세요.
- 이 결과는 "업데이트 내역"이라는 하나의 고정 공지 글 맨 위에 덧붙일 "날짜 블록"입니다. 형식: 첫 줄에 "■ ${md}"(날짜만, 제목·이모지 없이), 그 아래 불릿 3~6개. 각 불릿은 한 문장, 담백하게.
...
console.log('검토 후, "업데이트 내역" 고정 공지 글 맨 위에 이 블록을 붙여넣으세요.');
