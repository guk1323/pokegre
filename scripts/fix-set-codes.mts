...
// ⚠️ 하이픈이 늘 답은 아니다. SV2P·S10P·SMP2·CP1~6은 하이픈 없이 잘 걸린다.
//    P가 붙었다고 일괄로 바꾸지 말고 세트마다 카드 이름을 견줘 확인할 것.
//
// ⚠️ **코드를 찾아도 번호가 안 맞으면 고치면 안 된다.** ja-PMCG5("리더스 스타디움")는
//    스니커덩크가 PMCG-G1으로 부르는 것이 맞는데(세트 이름 「ジム拡張第1弾
//    「リーダーズスタジアム」」이 정확히 같다), 번호 체계가 다르다 —
//      우리 001 = タケシのズバット · 스니커덩크 001 = エリカのフシギダネ
//      (タケシのズバット는 저쪽에서 041번이다)
//    코드만 고치면 "걸리는데 다른 카드"가 되어 지금(0건)보다 나쁘다.
//    ja-PMCG6(→PMCG-G2)도 같아서 둘 다 그대로 뒀다(2026-08-07 카드 3장씩 견줘 확인).
