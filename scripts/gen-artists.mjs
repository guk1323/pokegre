...
// 스니커덩크엔 일러스트레이터 정보가 없어서, 작가 정보가 있는 이 무료 DB(영문판 기준)로
...
// ⚠️ 활동 시기는 **그 작가의 전체 활동**이지, 우리가 담은 카드의 범위가 아니다.
//    _eras.json은 원본 전체를 훑어 만든 것이라, 화면의 "2022~2026 활동"이 목록의
//    카드(2023~2026)보다 넓을 수 있다 — 388명 중 94명이 그렇다(2026-08-07 확인).
//    "활동"이라고 적었으니 이대로가 맞다. 담긴 카드 범위로 바꾸면 실제 활동 시기를
//    잃는다. 카드 수(count)는 담긴 수로 맞춰 뒀지만 시기는 다른 뜻이다.
function eraStr(e) {
  if (!e || !e.min) return ''
