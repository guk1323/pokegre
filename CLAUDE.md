...
③은 `scripts/ko-official-card-names.json`이 있어야 도는데 지금 레포에 없어 건너뛴다(일치 0/불일치 0으로 표시). ③이 제일 정확하다: `scripts/ko-official-card-names.json`(공식 사이트에서 받아 둔 39개 세트·2,083장의 정식 한글명)에 우리 결과가 있는지 본다. 없으면 우리가 틀린 것. 세트가 늘면 위 방법으로 더 받아 이 파일에 합치면 된다.
