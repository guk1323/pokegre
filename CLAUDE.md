...
## ⏱ 돌아가는 루틴 — 언제·무엇을·며칠 간격 (2026-08-08 코드 전수 확인)
...
⚠️ 아래 "언제"는 대부분 **기동 직후 + 매시간 확인**이고, 함수 안에서 "오늘 이미 했으면
   건너뛴다"를 보므로 **실제로는 표의 간격대로** 한 번만 한다. 배포하면 기계가 새로 떠서
   확인 시각의 위상이 바뀐다(그래서 "9시 정각"이 아니라 "9시 지나 첫 확인"이다).

| 무엇 | 언제 | 간격 | 비용 | 메모 |
|---|---|---|---|---|
| **덤프 받기**(위 ①) | 오전 9시 지나 첫 확인 | **매일** | 0 | 2026-08-08은 09:04에 돌았다 |
| **스니커덩크 힛카드** | 기동 20초 뒤 + 매시간 | **매일 · 세트 1개** | 0(무료) | 발매 6개월 내 일본판 중 **PPT로 힛카드 3장을 못 채우는** 세트. 후보 60장·요청 사이 700ms |
| **덤프가 못 채운 세트 줍기** | 덤프 직후 + 매시간 | 할 일이 있을 때만 | 하루 예산 100,000 | 새 세트가 들어온 날 말고는 **할 일이 없다** ↓ |
| **데이터 백업** | 기동할 때 + 24시간마다 | **매일** | 0 | `/data/backups/<날짜>` · **7일치** |
| **오늘 진열 팩 그림 데우기** | 기동 6분 뒤 + 24시간마다 | **매일** | 0 | 진열이 자정에 바뀌므로 따라 돈다 |
| **오늘의 상점 진열** | 한국시간 자정 | **매일** | 0 | 42개 중 일본 3 + 영문 3 |
| **출석 예산** | 한국시간 자정 | **매일** | 0 | 30,000GP · 7일 연속 보너스 |
| **그림 디스크 캐시 정리** | 기동할 때 + **6시간**마다 | | 0 | 400MB 넘으면 360MB로 깎는다 |
| **카드 이름 저장** | **5분**마다 | | 0 | 바뀐 게 있을 때만 쓴다 |

**⚠️ 세트별로 받던 순번제는 2026-08-08에 걷어냈다.** 카드 뽑기 42개를 7일에 도는 것과
나머지를 45일에 도는 것이 있었는데, 덤프 한 번이 매일 331개를 채우는 지금은 겹치기만 한다.
(45일짜리는 **한 번도 돈 적이 없었다** — "못 채운 세트가 있으면 쉰다"에 늘 하나가 걸려 있었다.)
남은 건 `warmPackPrices` = **"덤프가 못 채운 세트 줍기"** 하나뿐이다.
- 되살리고 싶어지면 **먼저 세어 볼 것.** 덤프가 이미 채운 세트를 또 받으면 크레딧만 나간다.
- `ja-SM1+`(썬&문)는 **저쪽에 값이 아예 없다**(덤프에 줄 1개·값 0). 서버가 덤프를 볼 때마다
  이런 세트를 스스로 골라 `ppt-state.json`의 `noPriceSets`에 적고 건너뛴다. 저쪽이 나중에
  값을 넣어 주면 저절로 풀린다.
...
| **한글판 시세** | PPT에 한글판이 없다 | **이베이 Browse API**(제목이 영어라 `"카드명 Korean Version"`으로 찾는다) | 검색할 때 · 6시간 캐시 | 0 |
...
이베이 한글판 6시간 · 감정 수량 낱장 12시간 · 그림(메모리) 7일 · 카드 뽑기 시세 24시간.
