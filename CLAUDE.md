...
**2칸째 = 남는 칸.** 더 오래 묵은 쪽이 가져간다.
| 무엇 | 간격 | 이 칸으로 얻는 것 |
|---|---|---|
| **감정 수량**(population) | **3일** | 팝수 조회 화면·카드 상세의 감정 수량. 감정은 몇 주 걸려 하루 이틀엔 안 바뀐다 |
| **등급별 낙찰**(ebay) | **4일** | 등급 시세. 천천히 쌓이고, 검색할 땐 실시간으로도 받는다 |

- 둘 다 한 번도 안 받았으면 **감정 수량이 먼저** 간다(카드를 열 때마다 2크레딧씩 나가는 쪽이라).
- 일주일이면 14칸 중 **11칸쯤** 쓴다(시세 7 + 감정 2~3 + 낙찰 1~2). 남는 칸은 비워 둔다.
...
| **카드 뽑기 시세 데우기** | 덤프 직후 + 매시간 | 사실상 **놀고 있다** ↓ | 오늘 73크레딧 | 덤프가 매일 330개를 채워 줘서 할 일이 남지 않는다 |
...
**⚠️ 카드 뽑기 시세 데우기(`warmPackPrices`)는 덤프가 생긴 뒤로 역할이 거의 없다.**
세트를 하나씩 불러 채우던 옛 방식인데, 지금은 덤프 한 번이 330개를 채운다.
운영 실측(2026-08-08): 세트 332개 중 **331개가 값이 있고 330개가 24시간 안에 갱신됐다.**
남은 일은 이 둘뿐이고, 둘 다 **아무리 두드려도 안 채워진다.**
- `ja-SM1+` — **저쪽(PPT)에 값이 아예 없다**(덤프에 줄 1개, 값 0). 6시간마다 영원히 재시도.
- `en-exu` — `en-ex10`과 **PPT 이름이 겹친다**("EX Unseen Forces"). 덤프는 앞의 것만 채운다.
- ⚠️ 그 결과 **45일 주기(`slowTargets`)가 한 번도 안 돈다** — "아직 못 채운 세트가 있으면
  쉰다"는 조건에 위 두 개가 영원히 걸려 있다. 어차피 덤프가 매일 다 갱신하므로 손해는 없다.
