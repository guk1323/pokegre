...
| 무엇 | 언제 | 간격 | 비용 | 메모 |
|---|---|---|---|---|
| **시세 통째 받기**(cards) | 오전 9시 지나 첫 확인 | **매일** | 크레딧 0 · 받기 몫 1칸 | 세트 330개·카드 36,000장이 한 번에 찬다. 2026-08-08은 09:04 |
| **레어도 낱말 뽑기** | 위 덤프를 받는 그 자리 | **매일** | 0 | `/data/rarity-terms.json` → `/api/local/rarity-terms`. 자동완성의 "리자몽 MUR" |
| **감정 수량**(population) | 남는 1칸 | **3일** | 크레딧 0 | 감정은 몇 주 걸려 하루 이틀엔 안 바뀐다 |
| **등급별 낙찰**(ebay) | 남는 1칸 | **4일** | 크레딧 0 | 천천히 쌓인다. 검색할 땐 실시간으로도 받는다 |
| **스니커덩크 힛카드** | 기동 20초 뒤 + 매시간 | **매일 · 세트 1개** | 0(무료) | 발매 6개월 내 일본판 중 **PPT로 힛카드 3장을 못 채우는** 세트. 후보 60장·요청 사이 700ms |
| **카드 뽑기 시세 데우기** | 덤프 직후 + 매시간 | 42개를 **7일**에 나눠 | 크레딧 씀 | 실패한 세트는 **6시간** 뒤 재시도. 채우기 예산 하루 100,000 |
...
| 무엇 | 언제 해야 하나 | 명령 |
|---|---|---|
| **배포** | 시킬 때만. **안 하면 위 ①이 옛 코드로 돈다** | `fly deploy -a pokegre` |
...
⚠️ **통째 받기를 손으로 하기 전에 서버가 오늘 이미 받았는지 먼저 볼 것.**
   하루 2회뿐인데 서버가 아침에 한 칸을 쓴다. 모르고 또 받아 429를 냈다(2026-08-08).
   ```
